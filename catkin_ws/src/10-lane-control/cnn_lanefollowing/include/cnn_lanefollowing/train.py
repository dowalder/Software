...
def validation(net, test_loader, criterion, device="cpu"):
    ...
    for data in test_loader:
        ...
        labels = -labels  # TODO: remove, only test
    ...
    rospy.loginfo("\ttest loss: %f" % avg_mse)
...
def train_net(net,
    ...
    rospy.loginfo("Moving the network to the device {}...".format(device))
    net.to(device)
    ...
    for epoch in range(num_epoch):
        for lbls, imgs in train_loader:
            if rospy.is_shutdown():
                raise RuntimeError("ROS is not running anymore.")
            ...
            if step % disp_interval == 0 and step != 0:
                rospy.loginfo("[%d][%d] training loss: %f" % (epoch, step, running_loss / val_interval))
                running_loss = 0

            if step % val_interval == 0 and step != 0:
                rospy.loginfo("[%d][%d] Performing validation..." % (epoch, step))
            ...
            if step % save_interval == 0 and epoch != 0:
                ...
                rospy.loginfo("[%d][%d] Saving a snapshot to %s" % (epoch, step, path))
